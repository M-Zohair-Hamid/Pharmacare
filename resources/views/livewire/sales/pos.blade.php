<div>
    ...
    @if ($lastSaleId)
        <div class="mb-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
            ...
            <a href="{{ route('sales.receipt', $lastSaleId) }}" target="_blank" class="ml-2 font-medium underline">Print receipt</a>
        </div>
    @endif
                            ...
                            @forelse ($medicines as $medicine)
                                <tr wire:key="pick-{{ $medicine->id }}" class="border-b border-slate-50 last:border-0">
                                    <td class="px-4 py-3 align-middle">
                                        ...
                                        <div class="text-xs text-slate-500">{{ $medicine->sku }}</div>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 align-middle text-right">
                                        <button wire:click="addToCart({{ $medicine->id }})" class="rounded-xl bg-teal-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-teal-700">Add</button>
                                    </td>
                                </tr>
                            @empty
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($cart as $medicineId => $item)
                        <div wire:key="cart-{{ $medicineId }}" class="flex items-center justify-between gap-2 border-b border-slate-50 pb-3">
                            <div class="min-w-0">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                />
                                <button wire:click="removeFromCart({{ $medicineId }})" class="text-xs text-rose-600 hover:text-rose-700">✕</button>
                            </div>
                        </div>
                    @empty
                ...
                <div class="mt-4 space-y-3">
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Customer</label>
                        <select wire:model="customerId" class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">
                    ...
                    <div>
                        ...
                        <select wire:model="paymentMethod" class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">
                ...
                <button
                    ...
                    class="mt-4 w-full rounded-xl bg-teal-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-teal-700 disabled:cursor-not-allowed disabled:opacity-50"
                >
                    Complete Sale
                </button>
            </div>
