                    ...
                    @forelse ($suppliers as $supplier)
                        <tr wire:key="sup-{{ $supplier->id }}" class="border-b border-slate-50 last:border-0">
                            ...
                            <td class="px-4 py-3 align-middle text-right">
                                <button wire:click="openEdit({{ $supplier->id }})" class="text-sm font-medium text-teal-700 hover:text-teal-800">Edit</button>
                                <button wire:click="delete({{ $supplier->id }})" wire:confirm="Delete {{ $supplier->name }}?" class="ml-3 text-sm font-medium text-rose-600 hover:text-rose-700">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="4" class="px-4 py-12 text-center text-sm text-slate-500">No suppliers yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
