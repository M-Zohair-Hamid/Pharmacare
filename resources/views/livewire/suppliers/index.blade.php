<div>
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">Suppliers</h1>
            <p class="mt-2 max-w-2xl text-sm text-slate-500 sm:text-base">Vendors you purchase medicines from.</p>
        </div>
    ...
    <input type="text" wire:model.live.debounce.300ms="q" placeholder="Search suppliers..."
            ...
            <table class="min-w-full text-left text-sm">
                <thead>
                    <tr class="border-b border-slate-100">
                        ...
                        <th class="whitespace-nowrap px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Contact</th>
                ...
                <tbody>
                    @forelse ($suppliers as $supplier)
                        <tr wire:key="sup-{{ $supplier->id }}" class="border-b border-slate-50 last:border-0">
                            <td class="px-4 py-3 align-middle">
                                <input type="checkbox" wire:model.live="selected" value="{{ $supplier->id }}" class="cursor-pointer rounded border-slate-300" />
                            </td>
                            <td class="px-4 py-3 align-middle">
                                <div class="font-medium text-slate-900">{{ $supplier->name }}</div>
                                <div class="text-xs text-slate-500">{{ $supplier->email ?? '—' }}</div>
                            </td>
                            <td class="px-4 py-3 align-middle text-slate-700">{{ $supplier->contact_person ?? '—' }}</td>
                            <td class="px-4 py-3 align-middle text-slate-700">{{ $supplier->phone ?? '—' }}</td>
                            <td class="px-4 py-3 align-middle text-right">
                                <button wire:click="openEdit({{ $supplier->id }})" class="cursor-pointer text-sm font-medium text-teal-700 transition-colors duration-150 hover:text-teal-800">Edit</button>
                                <button wire:click="delete({{ $supplier->id }})" wire:confirm="Delete {{ $supplier->name }}?" class="ml-3 cursor-pointer text-sm font-medium text-rose-600 transition-colors duration-150 hover:text-rose-700">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="px-4 py-12 text-center text-sm text-slate-500">No suppliers yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
            ...
            <div class="relative z-10 max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
                ...
                <form wire:submit="save" class="grid gap-4 p-5 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="contactPerson" class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" />
                    </div>
                    <div>
