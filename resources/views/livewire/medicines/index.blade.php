                ...
                <form wire:submit="save" class="grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div class="sm:col-span-2 lg:col-span-3 rounded-2xl border border-teal-100 bg-teal-50/50 p-3">
                            ...
                            <div class="grid gap-3 grid-cols-2 md:grid-cols-3">
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" wire:model.live="unitPrice" class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm bg-white" />
                                    @error('unitPrice') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                                </div>
                            </div>
                            <p class="mt-2 text-xs text-teal-700">
                                Unit price per tablet is suggested automatically (box price ÷ tablets per box), but you can edit it yourself — your value is kept until you change the box price or tablets per box again.
                            </p>
                        </div>
                    @else
                        <div>
                    ...
                    <div class="rounded-xl border border-amber-200 bg-amber-50/60 p-3">
                        @if ($editingId)
                            ...
                            <input type="date" value="{{ $expiryDate }}" readonly disabled class="w-full cursor-not-allowed rounded-xl border border-amber-200 bg-amber-100/60 px-3 py-2 text-sm text-amber-900" />
                            <p class="mt-1 text-[11px] text-amber-700">
                                Set automatically from the soonest-expiring batch below. Add or delete a batch to change it.
                            </p>
                        @else
                            <label class="mb-1 flex items-center gap-1 text-xs font-semibold uppercase tracking-wide text-amber-700">
                            ...
                            <p class="mt-1 text-[11px] text-amber-700">
                                This becomes the expiry of your initial stock batch.
                            </p>
                        @endif
                    ...
                    @if ($editingId)
                        <div class="sm:col-span-2 lg:col-span-3 rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                            ...
                            @if ($nextFefoSource)
                                <p class="mt-2 text-xs text-teal-700">
                                    Selling next (soonest expiry first):
                                    @if ($nextFefoSource['type'] === 'batch')
                                        batch <span class="font-semibold">{{ $nextFefoSource['batch_number'] ?? '#' . $nextFefoSource['id'] }}</span>,
                                    @else
                                        <span class="font-semibold">base stock</span> (no batch on record),
                                    @endif
                                    {{ $nextFefoSource['quantity'] }} unit(s), expires {{ $nextFefoSource['expiry_date']?->format('M j, Y') ?? '—' }}.
                                </p>
                            @endif
                                    ...
                                    <table class="min-w-full text-left text-xs">
                                        ...
                                        <tbody>
                                            @foreach ($editingMedicineBatches as $batch)
                                                <tr wire:key="batch-{{ $batch->id }}" class="border-b border-slate-50 last:border-0 {{ $nextFefoSource && $nextFefoSource['type'] === 'batch' && $nextFefoSource['id'] === $batch->id ? 'bg-teal-50/60' : '' }}">
                                                    <td class="px-3 py-2 text-slate-700">
                                                        {{ $batch->batch_number ?? '—' }}
                                                        @if ($nextFefoSource && $nextFefoSource['type'] === 'batch' && $nextFefoSource['id'] === $batch->id)
                                                            <span class="ml-1 inline-flex items-center rounded-full bg-teal-100 px-1.5 py-0.5 text-[10px] font-semibold text-teal-700">Selling next</span>
                                                        @endif
                                                    </td>
